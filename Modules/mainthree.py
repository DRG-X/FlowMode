...
while True:
    ...
    # If session hasn't started, wait
    if not session_started:
        time.sleep(0.1)
        continue
    ...
    # Fake key input for calibration (one-shot)
    key = 0
    ...
    # -----------------------------
    # RUN MODULES (NEW ARCH)
    # -----------------------------
    # Presence is only for AWAY logic
    presence_label = face_prescence_module.update(frame, now)

    # Always run head pose so head-turn is never ignored
    head_pose_label = head_pose_module.update(frame, now, key)

    # Run eye gaze whenever face is PRESENT
    # (but we do not let eye gaze override head pose distracted)
    eye_gaze_label = "NOT_CALIBRATED"
    if presence_label == "PRESENT":
        eye_gaze_label = eye_gaze_module.update(frame, key, now)

    # -----------------------------
    # FUSION LOGIC (HARD VETO)
    # -----------------------------
    # Priority:
    # 1) AWAY if presence says away
    # 2) DISTRACTED if head pose says distracted
    # 3) ATTENTIVE only if eyes say attentive (and head was not distracted)
    # 4) Otherwise DISTRACTED
    if presence_label != "PRESENT":
        final_state = "AWAY"
    else:
        if head_pose_label == "DISTRACTED":
